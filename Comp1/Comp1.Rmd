...
#l1 <- qchisq(0.9,2) * sqrt(lam1)
#l2 <- qchisq(0.9,2) * sqrt(lam2)
...
text(2,2.7, labels=TeX("$2\\sqrt{b}\\sqrt{\\lambda_1}$"))
lines(c(mu_x, x3+mu_x), c(mu_y, x4+mu_y), col = "red")
text(-1.8,2.2, labels=TeX("$2\\sqrt{b}\\sqrt{\\lambda_2}$"))

```

The major half-axis, in blue, shows $\sqrt{\lambda_1}b\frac{\boldsymbol{p_1}}{|\boldsymbol{p_1}|}$, i.e. the length $\sqrt{\lambda_1}b$ in the direction of the first eigenvector of $\Sigma$. Similarly, the minor half-axis, in red, shows $\sqrt{\lambda_2}b\frac{\boldsymbol{p_2}}{|\boldsymbol{p_2}|}$.

\textcolor{red}{Vi må spørre om hvorfor det som er tegnet ovenfor gir rett figur, mens det som er utledet i teorien er helt feil!?}
