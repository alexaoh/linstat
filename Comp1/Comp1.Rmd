...
v1 <- c(eigen(sigma)$vector[1,])
v2 <- c(eigen(sigma)$vector[2,])
...
f = function(t){
    return ((sqrt(lam1)*cos(theta)*cos(t)-sqrt(lam2)*sin(theta)*sin(t))+mu_x)
}

g = function(t){
    return (sqrt(lam1)*sin(theta)*cos(t)+sqrt(lam2)*cos(theta)*sin(t)+mu_y)
}
...
x1=(v1/length(v1)*sqrt(lam1))[1]
x2 =(v1/length(v1)*sqrt(lam1))[2]
