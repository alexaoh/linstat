...
plot(x,y, type = "l", lwd = 0.8, )
abline(v = c(qnorm(0.05), qnorm(0.95)))

# Coloring in the tails next.
left.x <- seq(-3, qnorm(0.05), by = 0.01)
right.x <- seq(qnorm(0.95), 3, by = 0.01)
left.y <- dnorm(left.x)
right.y <- dnorm(right.x)
polygon(x = c(qnorm(0.95), right.x, 3) , y = c(0, right.y, 0), col = "skyblue")
polygon(x = c(-3, left.x, qnorm(0.05)) , y = c(0, left.y, 0), col = "skyblue")
```
...
# Add chi-squared.
x.val <- seq(0, 10, by = 0.01)
y.val <- dchisq(x.val, df = 1)

hist(data, freq = F, main = "Std Gaussian^2 and Chisquared df = 1")
lines(x.val, y.val, col = "red")
